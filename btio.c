...
static bool l2cap_set(int sock, int sec_level, uint16_t imtu, uint16_t omtu,
        uint8_t mode, int master, int flushable, uint32_t priority) {
    if (imtu || omtu || mode) {
...
static bool sco_set(int sock, uint16_t mtu) {
    ...
    sco_opt.mtu = mtu;
    if (setsockopt(sock, SOL_SCO, SCO_OPTIONS, &sco_opt, sizeof(sco_opt)) < 0) {
        return false;
    }
...
static bool l2cap_get(int sock, BtIOOption opt1, va_list args) {
    ...
    if (!get_peers(sock, (struct sockaddr *) &src, (struct sockaddr *) &dst,
            sizeof(src)))
        return false;
    ...
    while (opt != BT_IO_OPT_INVALID) {
        ...
        case BT_IO_OPT_MASTER:
            ...
            *(va_arg(args, bool *)) = (flags & L2CAP_LM_MASTER) ? true : false;
            break;
...
static bool rfcomm_get(int sock, BtIOOption opt1, va_list args) {
    ...
    while (opt != BT_IO_OPT_INVALID) {
        ...
        case BT_IO_OPT_CHANNEL:
            *(va_arg(args, uint8_t *)) =
                    src.rc_channel ? src.rc_channel : dst.rc_channel;
            break;
        ...
        case BT_IO_OPT_MASTER:
            ...
            *(va_arg(args, bool *)) = (flags & RFCOMM_LM_MASTER) ? true : false;
            break;
        case BT_IO_OPT_HANDLE:
...
bool bt_io_set(int sock, struct set_opts *opts) {
    ...
    case BT_IO_L2CAP:
        return l2cap_set(sock, opts->sec_level, opts->imtu, opts->omtu,
                opts->mode, opts->master, opts->flushable, opts->priority);
    case BT_IO_RFCOMM:
...
static int create_handle(bool server, struct set_opts *opts) {
    ...
    case BT_IO_L2CAP:
        ...
        if (l2cap_bind(sock, &opts->src, opts->src_type, server ? opts->psm : 0,
                opts->cid) < 0)
            goto failed;
    ...
    case BT_IO_RFCOMM:
        ...
        if (rfcomm_bind(sock, &opts->src, server ? opts->channel : 0) < 0)
            goto failed;
...
int bt_io_connect(struct set_opts* opts) {
    ...
    switch (opts->type) {
    case BT_IO_L2CAP:
        err = l2cap_connect(sock, &opts->dst, opts->dst_type, opts->psm,
                opts->cid);
        break;
    case BT_IO_RFCOMM:
